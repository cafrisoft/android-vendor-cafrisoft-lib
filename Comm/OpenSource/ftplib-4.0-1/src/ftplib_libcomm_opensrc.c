 ...
 */
GLOBALDEF int FtpConnect(const char *host, int portNum, netbuf **nControl)
{
    int sControl;
...
static int FtpXfer_FileGet(const char* localfile, const char* path,
                           netbuf* nControl, int typ, int mode,
                           void* cbArg, Comm_NetUtils_FtpGetCallback cbfunc
          )
{
    int l, c;
    char* dbuf;
...
GLOBALREF int FtpGetWithCB(const char* outputfile, const char* path, char mode, netbuf* nControl, void* cbArg, Comm_NetUtils_FtpGetCallback cbfunc) {

    return FtpXfer_FileGet(outputfile, path, nControl, FTPLIB_FILE_READ, mode, cbArg, cbfunc);
}

/*
